...
int main(){
    ...
    std::vector<int> lowest_possible_color; lowest_possible_color.clear();
    for (int k=0; k<m; k++){
        lowest_possible_color.push_back(0);
    }
    ...
    while(i<m){
        ...
        if (t<used_colors.size()){
            ...
            //update lowest_possible_color
            for (int j=0; j<adjacency_list[current_vertex].size(); j++){
                int n_vertex=adjacency_list[current_vertex][j];
                std::vector<int> p_colors; p_colors.clear();
                for (int j=0; j<k; j++) {p_colors.push_back(0);}  

                for (int l=0; l<adjacency_list[n_vertex].size(); l++){
                    if (edge_colors[adjacency_list[n_vertex][l]]>=0) {p_colors[edge_colors[adjacency_list[n_vertex][l]]]=1;}
                }

                int tt=0; 
                while (p_colors[tt]==1) {tt++;}
                lowest_possible_color[n_vertex]=tt;
            }
        ...
        maxSat=0; maxDegree=0; maxLowestPossibleColor=-1;
        for (int j=0; j<m; j++){
            if (edge_colors[j]==-1){
                if (saturation[j]>maxSat){
                    ...
                    maxSat=saturation[j];
                    maxDegree= degrees[j];
                    maxLowestPossibleColor=lowest_possible_color[j];
                }
                else if (lowest_possible_color[j]>maxLowestPossibleColor && saturation[j]==maxSat){
                    maxSat_vertex=j;
                    maxSat=saturation[j];
                    maxDegree= degrees[j];
                    maxLowestPossibleColor=lowest_possible_color[j];
                } else if (lowest_possible_color[j]==maxLowestPossibleColor && saturation[j]==maxSat && degrees[j]>maxDegree){
                    maxSat_vertex=j;
                    maxSat=saturation[j];
                    maxDegree= degrees[j];
                    maxLowestPossibleColor=lowest_possible_color[j];
    ...
    //write solution JSON
     ...
     ofstream o("lpc_solution.json");
     o << std::setw(4) << solutionjson << std::endl;

     cout << "Number of Colors in LPC Algorithm: " << colors_counter << "\n";
