...
int main(){
    ...
    for (int i=0; i<m; i++){
        ...
        for (int j=0; j<i; j++){
            if (adj_matrix[i][j]==1){
                ...
                //edges_among_neighbors[i][j]=v_vec;
            }
        }
    }
    ...
    // for (int i=0; i<m; i++){
    //     std::cout << n_edges_among_neighbors[i] << "\n";
    //     if (n_edges_among_neighbors[i]>2809){g++;}
    // }
        ...
        if (min_edges_among_neighbors==(graph_size-1)*(graph_size-2)/2) {
            ...
            for (int i=0; i<adjacency_list[v].size(); i++){
                if (in_subgraph[adjacency_list[v][i]]==1){
                    ...
                    int x=v;
                    int y=c_v;
                    //reduce 1 from all vertices which form a triangle with x and y
                    for (int j=0; j<m; j++){
                        if (in_subgraph[j]==1 && adj_matrix[j][x]==1 && adj_matrix[j][y]==1){
                            n_edges_among_neighbors[j]--;
                        }
                    }
                }
            }
