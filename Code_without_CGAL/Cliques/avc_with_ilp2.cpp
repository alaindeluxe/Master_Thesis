...
int main(){
    ...
    //color clique
    int current_color=0;
    std::vector<int> in_clique_vec; in_clique_vec.clear();
    for (int i=0; i<m; i++){in_clique_vec.push_back(0);}
    for (int i=m-1; i>=0; i--){
        if (in_subgraph[i]==1){
    ...
    for (int i=0; i<200; i++){
        ...
        if (max_neighbors_in_subgraph>=current_lower_bound){
            ...
            for (int j=0; j<m; j++){
                if (in_subgraph[j]==1 and _vec[j]==1){
                    variables[j]=ilp_model.addVar(lb=edge_colors[j], ub=edge_colors[j], obj=0, vtype=GRB.INTEGER);
                }
                else if (in_subgraph[j]==1){
                    variables[j]=ilp_model.addVar(lb=0, ub=current_lower_bound-1, obj=0, vtype=GRB.INTEGER);
                }
            }

            for (int j=0; j<m; j++){
                if (j==current_vertex || in_subgraph[j]){
                    for (int l=0; l<m; l++){
                        if (l==current_vertex || in_subgraph[l]){
                            if (adjacency_matrix[j][l]==1 && (in_clique_vec[j]==0 || in_clique_vec[l]==0)){
        ...
        cout << "The lower bound is currently: " << current_lower_bound << "\n";
        cout << "We have this many vertices in the subgraph: " << graph_size << "\n";
