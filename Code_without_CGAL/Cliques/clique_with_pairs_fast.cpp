...
int main(){
    ...
    std::vector<int> in_subgraph_list; in_subgraph_list.clear();
    for (int l=0; l<m; l++){
        degrees.push_back(0);
        in_subgraph.push_back(0);
    }
    ...
    for (int i=0; i<m; i++){
        for (int j=i+1; j<m; j++){
            if (adj_matrix[i][j]==1){
                ...
                if (adjacency_list[i].size()<adjacency_list[j].size()){

                    for (int l=0; l<adjacency_list[i].size(); l++){
                        if (adj_matrix[j][adjacency_list[i][l]]==1){
                            in_subgraph[adjacency_list[i][l]]=1; graph_size++;
                            in_subgraph_list.push_back(adjacency_list[i][l]);
                            for (int o=0; o<l; o++){
                                if (adj_matrix[adjacency_list[i][l]][adjacency_list[i][o]]==1 && in_subgraph[adjacency_list[i][o]]==1){
                                    degrees[adjacency_list[i][l]]++; degrees[adjacency_list[i][o]]++;
                                }
                            }
                        } else {in_subgraph[adjacency_list[i][l]]=0;}
                    }

                } else {

                    for (int l=0; l<adjacency_list[j].size(); l++){
                        if (adj_matrix[i][adjacency_list[j][l]]==1){
                            in_subgraph[adjacency_list[j][l]]=1; graph_size++;
                            in_subgraph_list.push_back(adjacency_list[j][l]);
                            for (int o=0; o<l; o++){
                                if (adj_matrix[adjacency_list[j][l]][adjacency_list[j][o]]==1 && in_subgraph[adjacency_list[j][o]]==1){
                                    degrees[adjacency_list[j][l]]++; degrees[adjacency_list[j][o]]++;
                                }
                            }
                        } else {in_subgraph[adjacency_list[j][l]]=0;}
                    }

                }

                std::vector<int> initial_in_subgraph_list=in_subgraph_list;

                //is this graph a clique?
                int min_degree=m;
                int v=0;
                for (int l=0; l<graph_size; l++){
                    if (degrees[in_subgraph_list[l]]<min_degree){
                        v=in_subgraph_list[l];
                        min_degree=degrees[in_subgraph_list[l]];
                    }
                }
                ...
                //find clique in G'
                while (graph_size>max_clique_size-3 && clique_found==false){//maybe we can change the -5?
                    ...
                    if (min_degree==graph_size-1) {
                        ...
                        in_subgraph[v]=0;
                        //remove v from in_subgraph_list
                        in_subgraph_list.erase(in_subgraph_list.begin()+v_index);
                        graph_size--;
                ...
                if (clique_found==true){
                    for (int l=0; l<initial_in_subgraph_list.size(); l++){
                        if (in_subgraph[initial_in_subgraph_list[l]]==0){
                            bool in_clique=true;
                            int c=graph_size;
                            int c_v=initial_in_subgraph_list[l];
                            for (int o=0; o<adjacency_list[c_v].size(); o++){
                                if (in_subgraph[adjacency_list[c_v][o]]){
                                    c--;
                                }
                            }
