...
int main(){
    ...
    ifstream file("instances/vispecn71708.instance.json");
    ...
    std::cout << "Current number of colors: " << colors_counter-1 << "\n";
    ...
    for (int i=0; i<30; i++){//b=m
        int current_v=vertex_with_color[i].first;
        ...
        while (last_neighbor_index<current_neighbors_with_color.size()){
            ...
            if (everybody_best_color){
                ...
                used_colors[edge_colors[current_v]]--;
                if (used_colors[edge_colors[current_v]]==0){
                    colors_counter--;
                }
    ...
    //check that every edge has the lowest possible color
    for (int i=0; i<m; i++){
        int current_vertex=vertex_with_color[i].first;
        std::vector<int> possible_colors; possible_colors.clear();
        for (int j=0; j<k; j++) {possible_colors.push_back(0);}
        for (int j=0; j<adjacency_list[current_vertex].size(); j++){
            possible_colors[edge_colors[adjacency_list[current_vertex][j]]]=1;
    ...
    //sort color classes such that largest color classes have small ID-number
