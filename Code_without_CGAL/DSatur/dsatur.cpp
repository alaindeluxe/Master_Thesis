...
int main(){
    ...
    ifstream file("instances/visp44362.instance.json");
    ...
    sort(degree_intersection_graph.begin(),degree_intersection_graph.end(), myComp_low_to_high);

    //for (int i=0; i<m; i++){
    //    std::cout << degree_intersection_graph[i].second << "\n";
    //}
    ...
    std::vector<int> new_color_introduced; new_color_introduced.clear();
    ...
    while(i<m){
        ...
        if (t<used_colors.size()){
            ...
            new_color_introduced.push_back(i);
            t=0;
    ...
    cout << "New colors introduced: " << "\n";

    //for (int i=0; i<new_color_introduced.size(); i++){
    //    cout << new_color_introduced[i] << " ";
    //}
