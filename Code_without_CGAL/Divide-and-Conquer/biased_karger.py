...
def dac(cons_edges):
    ...
    if (edge_counter<=50):
        ...
        #We use a biased implementation of Karger's algorithm
        current_size = 0
        subsets = {}
        parent = {}
        rank = {}
        ...
        for i in range(0,m):
            if (cons_edges[i]==1):
                for j in range(0,m):
                    if (cons_edges[j]==1):
                        #count degrees
                        weights[i,j] = current_degrees[i]+current_degrees[j]
                        weights[j,i] = current_degrees[i]+current_degrees[j]
        ...
        while (current_size>2):
            ...
            for i in range(0,m):
                if (cons_edges[i]==1):
                    for j in range(0,m):
                        if (cons_edges[j]==1):
                            if (weights[i,j]>max_weight and (not set(subsets[i])==set(subsets[j]))):
                                edge_0 = i
                                edge_1 = j
                                max_weight = weights[i,j]

            #contract max_weight edge
            current_size = current_size-1

            #add subset[i] to subset[j] and subset[j] to subset[i]

            for i in range(0,len(subsets[edge_0])):
                subsets[edge_1].append(subsets[edge_0][i])
            subsets[edge_1] = [x for x in subsets[edge_1] if subsets[edge_1].count(x) == 1]
            current_degrees[edge_1]=0
            for i in range(0,len(subsets[edge_1])):
                current_degrees[edge_1] = current_degrees[edge_1]+degrees[subsets[edge_1][i]]
            for i in range(0,len(subsets[edge_1])):
                current_degrees[subsets[edge_1][i]] = current_degrees[edge_1]
                subsets[subsets[edge_1][i]]=subsets[edge_1]

            for j in range(0, len(subsets[edge_1])):
                subsets[edge_0].append(subsets[edge_1][j])
            subsets[edge_0] = [x for x in subsets[edge_0] if subsets[edge_0].count(x) == 1]
            current_degrees[edge_0]=0
            for i in range(0,len(subsets[edge_0])):
                current_degrees[edge_0] = current_degrees[edge_0]+degrees[subsets[edge_0][i]]
            for i in range(0,len(subsets[edge_0])):
                current_degrees[subsets[edge_0][i]] = current_degrees[edge_0]
                subsets[subsets[edge_0][i]]=subsets[edge_0]

            #update weights of the edges
            for i in range(0,m):
                if (cons_edges[i]==1):
                    for j in range(0,m):
                        if (cons_edges[j]==1):
        ...
        l=0
        while (cons_edges[l]==0):
            l=l+1

        for i in range(0,len(subsets[l])):
            left_cons_edges[i]=1
            right_cons_edges[i]=0
        for i in range(0,m):
            if (left_cons_edges[i]==0 and cons_edges[i]==1):
