...
int main(){
    ...
    ifstream file("dimacs_instances/dsjc1000.1.col.txt");
    ...
    while(num_v<=n){
        ...
        if (t<used_colors.size()){
            ...
            //cout << "qq" << "\n";
            for (int j=0; j<adjacency_list[current_vertex].size(); j++){
                //cout << adjacency_list[current_vertex][j] << "\n";
        ...
        for (int a=0; a<3; a++){
            ...
            for (int ii=0; ii<num_v/5; ii++){//b=m
    ...
    //write solution JSON
     ...
     solutionjson["instance"] = "DSJC500-5";
     ...
     ofstream o("dsatur_impr4_new_color_solution.json");
     o << std::setw(4) << solutionjson << std::endl;

     std::cout << "Number of Colors in DSatur w.Impr.4 (New Color) Algorithm: " << colors_counter << "\n";
